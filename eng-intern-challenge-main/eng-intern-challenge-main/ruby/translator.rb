...
def braille_to_english(braille)
  braille.split.map { |b| braille_to_english[b] || '?' }.join
end

def english_to_braille(text)
  text.downcase.chars.map { |c| braille_alphabet[c] || '?' }.join(' ')
end

input = ARGV.join(' ')
is_braille = input.chars.all? { |c| 'O. '.include?(c) }

result = is_braille ? braille_to_english(input) : english_to_braille(input)
puts result
